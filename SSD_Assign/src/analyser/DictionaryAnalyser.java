...
public class DictionaryAnalyser extends BaseAnalyser {
	...
	/**
	 * Clear all the existing known and unknown  word Collections 
	 * 
	 * Reading the word by calling redNextWord() which is in the BaseAnalyser and store in the nextWord
	 * 
	 * Process the available word
	 * 
	 * Checking if nextWord is contains in dictionary or not if contains then add to knownWord else add to unknownWord
	 * 
	 * @throws IOException if the named file could not be found and loaded.
	 */
	@Override
	public void performAnalysis(String filename) throws IOException {
		...
		while (nextWord != null) {
			...
			// if it is then record as a known word, otherwise record as an unknown words
